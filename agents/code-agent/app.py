...
class ModelService:
    def __init__(self):
        ...
        self.context_tokens = max(2048, int(os.getenv("MODEL_CONTEXT_TOKENS", "8192")))
        self.batch_tokens = max(64, min(self.context_tokens, int(os.getenv("MODEL_BATCH_TOKENS", "256"))))
        self.max_input_tokens = max(1024, int(os.getenv("MODEL_MAX_INPUT_TOKENS", "5600")))
        self.source_new_tokens = max(128, int(os.getenv("SOURCE_MODEL_MAX_NEW_TOKENS", "480")))
        self.repair_new_tokens = max(160, int(os.getenv("REPAIR_MODEL_MAX_NEW_TOKENS", "560")))
